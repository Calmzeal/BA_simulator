...
if __name__ == "__main__":
    ...
    # parameters to test
    for num_nodes in [60, 100, 200, 300, 400, 500, 600]:
        latency_min = 0.08
        latency_max = 2
        while True:
            latency = round((latency_min + latency_max)/2.,2)
            test_params.num_nodes = num_nodes
            test_params.latency = latency
            # test_params.out_degree = out_degree
            # extra_send.withhold = withhold
            ...
            average_diameter = round(sum(attack_last_time) / repeats, 2)
            print(f"{test_params},average_diameter:{average_diameter}")
            # samples = 2
            # print(list(map(lambda percentile: attack_last_time[int((repeats - 1) * percentile / samples)],
            #                range(samples + 1))))
            end = time.time()
            print("Executed in %.2f seconds" % (end - begin))
            if average_diameter > 8.05:
                latency_max = latency
            elif average_diameter < 7.95:
                latency_min = latency
            else:
                break