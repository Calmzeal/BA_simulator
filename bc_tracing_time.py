...
import copy
import time
from strategy_fixed_peer_latency3 import StrategyFixedPeerLatency
...
class Parameters:
    ...
    def __repr__(self):
        return f"latency:{self.latency},num_nodes:{self.num_nodes},out_degree:{self.out_degree},withhold:{self.withhold},recent_timeout:{self.recent_timeout},attack_timeout:{self.attack_timeout},extra_send:{self.extra_send}"
...
class Simulator:
    ...
    def run_test(self):
        ...
        while timestamp < self.env.termination_time:
            ...
            if trigger_adversary_action:
                if event is None:
                    self.attack_execution(adversary_mined,timestamp)
                else:
                    self.attack_execution(False,timestamp)
    ...
    def attack_execution(self,adversary_mined,timestamp):
...
if __name__=="__main__":
    ...
    test_params.termination_time = 1.0
        ...
        for attack_timeout in [0]:
            for timeout in [3.35]:
                for extra_send in [-1]:
                    ...
                    test_params.recent_timeout = timeout
                    test_params.attack_timeout = attack_timeout
